  ...
  return {
    visitor: {
      ImportDeclaration: function ImportDeclaration(path, state) {
        var node = path.node,
          dec = void 0;
        var src = path.node.source.value; // Don't do anything if not a relative path
        // if if not a relative path then a module
        if (src[0] !== "." && src[0] !== "/") return;

        //Inder :  Specific Directories only       
        const currentFilePath = state.file.opts.filename.replace(
          state.file.opts.root + "/",
          ""
        );   
        //Ignore node_modules
        if((currentFilePath && currentFilePath.indexOf("node_modules") === -1)){
        //Inder :  Specific Directories only ends

        var addWildcard = false, // True if should perform transform
        ...
        var filenameRegex = new RegExp(".+"); // in the above case we need to remove the trailing /*
        if (isExplicitWildcard) {
          ...
          filenameRegex = filenameGlob.replace(/[*\.\(\[\)\]]/g, function (
            character
          ) {
            switch (character) {
              case "*":
        ...
        for (var i = node.specifiers.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            _fs3.default.existsSync(dir) &&
            !_fs3.default.statSync(dir).isFile()
          ) {
            addWildcard = true;
          ...
          if (t.isImportSpecifier(dec) && isExplicitWildcard) {
            ...
            addWildcard = true; // Remove the specifier
            node.specifiers.splice(i, 1);
          }
        ...
        if (addWildcard) {
          ...
          try {
            ...
            for (var i = 0; i < r.length; i++) {
              ...
              var _path$parse = _path3.default.parse(r[i]),
                ...
                ext = _path$parse.ext;
              if (
                exts.indexOf(ext.substring(1)) > -1 &&
                filenameRegex.test(_name)
              ) {
                files.push(r[i]);
                isDirs.push(!ext);
              }
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            var file = files[i];
            var isDir = isDirs[i]; // Strip extension
            ...
            if (state.opts.nostrip !== true) {
              name =
                "./" + _path3.default.join(src, _path3.default.basename(file));
            } else {
              name = "./" + _path3.default.join(src, file);
            } // Special behavior if 'filterNames'
            if (filterNames.length > 0) {
              var _importDeclaration = t.importDeclaration(
            ...
            if (wildcardName !== null) {
              var thing = t.expressionStatement(
