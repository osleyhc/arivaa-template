...
export default class Main extends Component {
  ...
  getMin() {
    ...
    if (typeof min !== 'undefined' && typeof min !== 'number') {
      console.warn(
        'Min should be a number, Taking default as ' + this.config.min
      );
  ...
  getMax() {
    ...
    if (typeof max !== 'undefined' && typeof max !== 'number') {
      console.warn(
        'Max should be a number, Taking default as ' + this.config.max
      );
  ...
  getStep() {
    ...
    if (typeof step !== 'undefined' && typeof step !== 'number') {
      console.warn(
        'Step should be a number, Taking default as ' + this.config.max
      );
    ...
    switch (this.getType()) {
      ...
      case 'range':
        if (JSON.stringify(nextProps.value) !== JSON.stringify(value)) {
          this.setState({
            value: nextProps.value || [this.getMin(), this.getMax()],
          });
        }
        break;
    }
