...
import ActionSheetManager, {
  ActionSheetItem,
} from '../../external-libs/react-native-action-sheet-component/src/index';
...
export function getPropsForCustomShareActionSheet(config) {
  ...
  const Item = (props) => {
    ...
    return (
      ...
      >
        <View
          style={[styles.thumbnailView, itemPropStyles.itemTouchableContainer]}
        >
          <Thumbnail
            borderRadius={0}
            icon={props.icon}
            ...
            style={[
              styles.thumbnail,
              itemPropStyles.thumbnail,
              props.thumbnailProps ? props.thumbnailProps.style : null,
            ]}
          />
          {props.text && props.text !== '' ? (
            <Text
              numberOfLines={props.numberOfLines || 1}
              style={[styles.thumbnailText, itemPropStyles.itemText]}
            >
              {props.text}
            </Text>
          ) : null}
  ...
  if (native) {
    return ActionSheet.showActionSheetWithOptions(
      ...
      (buttonIndex) => {
        ...
        if (
          options[buttonIndexNumber].action &&
          options[buttonIndexNumber].action instanceof Function
        ) {
          options[buttonIndexNumber].action(
            options[buttonIndexNumber],
            buttonIndexNumber
          );
        }
...
function showShareActionSheetWithOptions(config) {
  ...
  if (native) {
    ...
    return ActionSheetManager.show(
      getPropsForCustomShareActionSheet(config),
      () => {
        if (config.onShow instanceof Function) {
          config.onShow();
        }
